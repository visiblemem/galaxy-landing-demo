...
float waveHeight(vec2 p,float tm){
 ...
 return broad+chop+vWake+inner;
}
vec3 getNormal(vec2 p,float tm){float e=.035;float h=waveHeight(p,tm);float hx=waveHeight(p+vec2(e,0),tm);float hz=waveHeight(p+vec2(0,e),tm);return normalize(vec3(h-hx,e,h-hz));}
vec3 skyColor(vec3 rd){
 float y=max(rd.y,0.);vec3 horizon=vec3(.38,.57,.62);vec3 zenith=vec3(.08,.19,.25);vec3 col=mix(horizon,zenith,pow(y,.55));
 vec3 sunDir=normalize(vec3(-.38,.42,.82));float sun=pow(max(dot(rd,sunDir),0.),520.);float glow=pow(max(dot(rd,sunDir),0.),18.);
 col+=vec3(1.,.82,.56)*sun*3.2+vec3(.9,.62,.38)*glow*.28;
...
void main(){
 ...
 if(rd.y>=-.018){col=skyColor(rd);}
  ...
  float sparkle=pow(max(dot(reflect(normalize(vec3(.32,-.8,-.45)),n),-rd),0.),72.);col+=vec3(1.,.86,.62)*sparkle*.9;
  float micro=step(.994,hash(floor((wp+tm*.2)*18.)));col+=micro*vec3(.8,.95,1.)*.12;
 }
 ...
 try{
  ...
  function resize(){const d=Math.min(devicePixelRatio||1,innerWidth<700?1:1.25),w=Math.floor(innerWidth*d),h=Math.floor(innerHeight*d);if(canvas.width!==w||canvas.height!==h){canvas.width=w;canvas.height=h;canvas.style.width=innerWidth+'px';canvas.style.height=innerHeight+'px';gl.viewport(0,0,w,h)}}
